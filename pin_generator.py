...
For each row, builds a pin (2:3 ratio, Pinterest's preferred size):
    ...
    [ same image again ]
    [ small "Akash" watermark badge, bottom-right corner ]
...
WATERMARK_TEXT = os.environ.get("WATERMARK_TEXT", "Akash")
JPEG_QUALITY = int(os.environ.get("JPEG_QUALITY", "85"))

# --- Video pins -------------------------------------------------------------
# If enabled, every generated pin image is ALSO turned into a short .mp4
# where the image just sits on screen for a random duration (picked from
# VIDEO_DURATIONS each time) - Pinterest accepts video pins the same way it
# accepts image pins, and video sometimes gets more reach.
...
GITHUB_REPOSITORY = os.environ.get("GITHUB_REPOSITORY", "").strip()   # e.g. "Akash/PinterestPins"
...
def get_utm_query() -> str:
    ...
    leading '?'), e.g. "utm_source=pinterest&utm_medium=social&utm_campaign=akash".
    Priority:
...
def build_pin_video(image_path: str, out_path: str, duration: int, fps: int = 30) -> None:
    """
    Turns a single pin JPEG into a short silent .mp4 - the image just holds
    still on screen for `duration` seconds. Requires ffmpeg to be installed
    and on PATH (already present on GitHub Actions ubuntu-latest runners;
    on your own machine install it from https://ffmpeg.org/download.html).
    """
    cmd = [
        "ffmpeg", "-y",
        "-loop", "1",
